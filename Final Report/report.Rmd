...
We can use the softened classifications on our original set of $n$ observations to quantify the instability of the algorithm, $\hat I$ over all $n$ measurements. An instability measure of $\hat I = 0$ indicates that the algorithm is perfectly stable over the testing set, while larger values indicate greater instability.
...
```{r toyDecisionBoundaries, fig.cap = "500 Decision Boundaries from Perturbed Datasets"}
...
cbind(glm.tbl, nb.tbl, rf.tbl) %>%
  kable("latex", caption = "95\\% confidence intervals from Gaussian perturbation, for each classifier", booktabs = T) %>%
  kable_styling(font_size = 10)
```


```{r, echo = F}
# "95% confidence intervals from Gaussian perturbation for each classifier"
# glm.instab <- 0.09204126
# nb.instab <- 0.02805608
# 
# %>%
