...
For a new measurement $\mathbf u$ (i.e., a measurement that is neither in the training nor test set), a softened classification can be obtained by generating $B$ perturbations of $\mathbf u$ using the known measurement error and averaging over these predictions as well.

We can use the softened classifications on our original set of $n$ observations to quantify the instability of the algorithm, $\hat I \in [0, \frac 12]$ over all $n$ measurements. An instability measure of $\hat I = 0$ indicates that the algorithm is perfectly stable over the testing set, while larger values indicate greater instability. Maximum instability of $\hat I = \frac 12$ is attained when each observation has $\frac B2$ predictions each of 0 and 1.

$$
\hat I = \frac 1n \sum_{i = 1}^n \sqrt { \frac1B \sum_{j = 1}^B (f(\mathbf W^{(j)}_i) - \hat y_i)^2 }
$$
...
classifier_instab <- function(preds) {
  ...
  mean(sqrt(rowMeans((preds - y_matrix)^2)))
}
```
...
Figure \@ref(fig:toyDecisionBoundaries) depicts the decision boundaries resulting from fitting the classifier to each of the 500 sets of perturbed measurements, while Figure \@ref(fig:toySoftClassifier) depicts the softened classifications, $\{ \hat y_i \}_{i = 1}^{500}$, resulting from averaging predictions over these decision boundaries. The algorithm is generally stable, so most predictions are unaffected by the perturbations. Near the middle of the plot, some points are visible with softened classifications[^softened].

[^softened]: Note that it is also possible to start with a soft classifier and obtained further softened results by averaging over the $B$ predictions. For the sake of simplicity of illustration, a hard classifier is used here.

```{r toyDecisionBoundaries, cache = T, fig.cap = "Decision Boundaries and Softened Classifications via Perturbed Datasets"}
...
nb.tbl <- sprintf("(%0.4f, %0.4f)", p.nb.lower, pmin(1, p.nb.upper)) %>%
  matrix(nrow = length(nb.meas), dimnames = list(names(nb.meas), "95% Perturbed CI (NB)"))

rf.tbl <- sprintf("(%0.4f, %0.4f)", p.rf.lower, pmin(1, p.rf.upper)) %>%
  matrix(nrow = length(rf.meas), dimnames = list(names(rf.meas), "95% Perturbed CI (RF)"))

cbind(glm.tbl, nb.tbl, rf.tbl) %>%
  kable("latex", booktabs = T, caption = "95% confidence intervals from Gaussian perturbation for each classifier") %>%
