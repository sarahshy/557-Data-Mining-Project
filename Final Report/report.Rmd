---
title: "Using Measurement Error for Good (e.g. curing corona): Gaussian Perturbation for Classification Stability and Uncertainty Quantification"
...
We begin by motivating the subject with a toy example in Section \@ref(motivation). In Section \@ref(pipeline), we introduce the Gaussian perturbation pipeline for stability and uncertainty quantification. In Section \@ref(toyexample), we apply the pipeline to the previously instroduced simulated toy data. In Section \@ref(astro), we present a real-world application to Astronomy data. Finally, we discuss the results and future directions in Section \@ref(discussion).
...
We created 500 perturbed sets using the original dataset and the known measurement error on each observation. Note that this example differs from the toy example in that each observation has its own measurement error. The variance used to perturb each observation varies.
...
Logistic regression and random forest received a similar instability score, in which case we can choose the algorithm that simply performs better on new unseen data. However, we run into trouble when comparing their confidence intervals. All five of the performance metrics are bounded on the unit interval. Since random forest performs very well on this data, the confidence intervals exceed 1. For this reason, we truncated the confidence intervals at 1 in table \@ref(tab:astroCI). Therefore, the method we used for calculating confidence intervals may not make sense if the performance measures fall near 0 and 1. Further consideration is needed to better handle these confidence intervals as this is an obvious limitation. One potential solution is to assume a distribution for these quantities that is appropriately bounded and incorporate the estimated variance from the perturbed measurements.


# Discussion

We have demonstrated how Gaussian perturbation can be used to measure algorithm stability and quantify uncertainty in classification tasks. Utilizing known measurement error, these measures are effectively functions of the known error. Moreover, we discussed how this method can be used for algorithm selection and calculating class probabilities for each observation.

In the future, we would like to explore alternative methods for computing confidence intervals, as mentioned in the previous section. In addition, we are interested in exploring how this pipeline translates to a clustering task, where performance measures are relatively nebulous. Finally, we are interested in using this pipeline to potentially improve the performance of a classifier through ensemble learning methods.
