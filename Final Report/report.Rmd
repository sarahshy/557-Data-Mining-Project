...
We can use the softened classifications on our original set of $n$ observations to quantify the instability of the algorithm, $\hat I \in [0, \frac 12]$, over all $n$ measurements. An instability value of $\hat I = 0$ indicates that the algorithm is perfectly stable over the testing set, while larger values indicate greater instability. The greatest possible instability of $\hat I = \frac 12$ would be attained if the predictions for each individual observation were evenly split between 0 and 1.
