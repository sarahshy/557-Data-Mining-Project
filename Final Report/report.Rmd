...
sprintf("(%0.4f, %0.4f)", lower, pmin(1, upper)) %>%
  matrix(nrow = length(glm.meas), dimnames = list(names(glm.meas), "95% Perturbed CI")) %>%
...
sprintf("(%0.4f, %0.4f)", lower, pmin(1, upper)) %>%
  matrix(nrow = length(nb.meas), dimnames = list(names(nb.meas), "95% Perturbed CI")) %>%
  kable("latex", booktabs = T) %>%
  kable_styling(font_size = 10)
```
...
sprintf("(%0.4f, %0.4f)", lower, pmin(1, upper)) %>%
  matrix(nrow = length(rf.meas), dimnames = list(names(rf.meas), "95% Perturbed CI")) %>%
  kable("latex", booktabs = T) %>%
  kable_styling(font_size = 10)
```
