...
ggplot(bind_rows(
  sim_results_me %>% add_column(observed = "True Values"),
  sim_results_noisy %>% add_column(observed = "Measured Values")
)) +
  geom_boxplot(aes(x = observed, y = accuracy)) +
  xlab("Starting Dataset") +
  ylab("Accuracy")
```

- The standard deviation when starting with **true** values (what we want to estimate) is `r sprintf("%0.5f", sd(sim_results_me$accuracy))`.
- The standard deviation when starting with a set of **noisy measurements** is `r sprintf("%0.5f", sd(sim_results_noisy$accuracy))`.
...
names(counts) <- c("n_votes_gal", "n_votes_star", "prob_gal")
...
- Improve predictions via GP and stacking
